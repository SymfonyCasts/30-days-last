...
inside the `body`, paste:

[[[ code('0cc9700c0d') ]]]

If we go over and refresh, you can see what I mean: it just works. Well, *visually*.
But if we click, nothing happens.
...
Flowbite handles initializing on the first page load. Then anytime we navigate
with Turbo, this method will be called and will reinitialize the listeners.
Or if we do something inside a Turbo frame, *this* will be called.
...
We're going to attach this controller to an `input` element. In `connect()`,
this initializes the date picker and passes `this.element`. The `format` matches
the default format that the Symfony `DateType` uses. And `autohide` makes
the date picker close when you choose a date, which I like.
...
on the page that's open, then use that as the container. Else use the normal `body`:
...
bottom, I'll paste in a new private method called `isClickInElement()`:

[[[ code('75a3765b3c') ]]]

If you pass this a click event, it will look at the physical dimensions of this
element and see if the click was inside.
...
A bit more logic, but a bit smarter. Try it. Open the modal and if we click down
here... the calendar closes - which is correct - but the modal stays open. Love that!
